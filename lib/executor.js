 ...
 * @returns {Promise<{ txid: string | null, usdcReceived: number, exitPrice: number }>}
 *   exitPrice — USDC per token implied by this sell (usdcReceived / tokenAmount); use for PnL so closes
 *   match Jupiter fills, not CoinGecko spot (which can differ or map to the wrong asset for a symbol).
 */
export async function sell({ symbol, mint, tokenAmount, slippageBps, wallet, connection, dryRun, currentPrice }) {
  ...
  const exitPrice = tokenAmount > 0 ? usdcReceived / tokenAmount : 0;

  console.log(`[executor] SELL ${symbol}: tx=${txid}, received=$${usdcReceived.toFixed(2)} USDC`);
  return { txid, usdcReceived, exitPrice };
}
