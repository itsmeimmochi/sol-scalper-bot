...
function rowsToTokens(rows) {
  return rows.map((r) => {
    const token = { symbol: r.symbol, geckoId: r.gecko_id, mint: r.mint };
    if (r.enabled !== undefined && r.enabled !== null) {
      token.enabled = Boolean(r.enabled);
    }
    return token;
  });
}

/** @param {{ enabled?: boolean }} token */
export function tokenAllowsBuy(token) {
...
/**
 * Scan universe: enabled tokens plus disabled tokens with an open position in the current lane.
 * @param {boolean} isSimulated dry_run lane
 */
async function loadTradingTokensForScan(isSimulated) {
  const res = await getPool().query(
    `
    SELECT t.symbol, t.gecko_id, t.mint, t.enabled
    FROM trading_tokens t
    WHERE t.enabled = true
       OR EXISTS (
         SELECT 1
         FROM open_positions op
         WHERE op.symbol = t.symbol AND op.is_simulated = $1
       )
...
export async function loadConfig() {
  ...
  const tokens = await loadTradingTokensForScan(Boolean(row.dry_run));

  if (tokens.length === 0) {
    throw new Error('No trading tokens configured (enable rows in trading_tokens). Run: npm run db:seed');
  }
...
/**
 * Merge auto-discovered tokens into trading_tokens.
 * New rows are enabled; existing rows keep their enabled flag (so DB disabled=false blocks re-enable).
