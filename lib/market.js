...
/**
 * CoinGecko hourly market_chart sometimes returns duplicate timestamps for the same UTC hour.
 * A single INSERT … ON CONFLICT must not list the same (gecko_id, ts) twice — Postgres error 21000.
 * Canonical hour buckets via floorToHourUtc so keys match what we persist.
 */
function dedupeCandlesByHour(candles) {
  const byHourMs = new Map();
  for (const c of candles) {
    if (!(c.ts instanceof Date) || !Number.isFinite(c.ts.getTime())) {
      continue;
    }
    const floored = floorToHourUtc(c.ts);
    if (!Number.isFinite(floored.getTime())) {
      continue;
    }
    const key = floored.getTime();
    byHourMs.set(key, { ...c, ts: floored });
  }
  return Array.from(byHourMs.values()).sort((a, b) => a.ts - b.ts);
}

async function upsertHistoryCandles(geckoId, points) {
  const candles = dedupeCandlesByHour(
    points
      .map((p) => {
  ...
  const sql = `
    ...
    SELECT DISTINCT ON (gecko_id, ts)
      gecko_id, ts, open, high, low, close, volume, source
    FROM (
      VALUES ${placeholders.join(',')}
    ) AS v(gecko_id, ts, open, high, low, close, volume, source)
    ORDER BY gecko_id, ts, close DESC
