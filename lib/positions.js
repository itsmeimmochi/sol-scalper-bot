...
/**
 * Align live open_positions with wallet: drop stale rows below dust (tracked mints only); adopt wallet-held tracked tokens.
 * Live rows whose mint is not in the current token universe are kept — not pruned as dust (no price/balance for that mint in this pass).
  ...
  try {
    ...
    const pruneSymbols = liveRows
      .map((row) => {
        ...
        if (geckoId == null) {
          console.warn(
            `[positions] Wallet reconcile: keeping live row ${row.symbol} (mint ${row.mint}) — not in tracked token universe; skipping dust prune (on-chain holdings may still exist)`
          );
          return null;
        }
        const price = currentPrices.get(geckoId);
...
function openedAtFromPayloadOrNow(payload) {
  if (typeof payload.openedAt === 'string' && payload.openedAt.trim() !== '') {
    return payload.openedAt;
  }
 ...
 * @param {{ symbol: string, mint: string, entryPrice: number, sizeUsdc: number, tokenAmount: number, isSimulated?: boolean, openedAt?: string }} payload — openedAt should be swap time from buy()
 */
export async function reconcileOpenAfterOnChainBuy(payload) {
  const openedAtNew = openedAtFromPayloadOrNow(payload);
...
export async function persistBuyOpenWithRetries(payload) {
  ...
  try {
    ...
    const openedAt = openedAtFromPayloadOrNow(payload);
...
export async function openPosition(position) {
  ...
  const openedAt = openedAtFromPayloadOrNow(position);
  const pool = getPool();
  await pool.query(
    `INSERT INTO open_positions (symbol, mint, entry_price, size_usdc, token_amount, opened_at, is_simulated)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
